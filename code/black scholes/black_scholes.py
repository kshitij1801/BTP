...
#                x_b_element    = tf.constant(np.array([[grid_x[ex]], [grid_x[ex+1]]]))
                ...
                if var_form == 0:
                    U_NN_element = tf.convert_to_tensor([[\
                                    ...
                                    *(d1tu_NN_quad_element + riskf_rate * x_quad_element * d1xu_NN_quad_element + 0.5*(sigma**2)*(x_quad_element**2)*d2xu_NN_quad_element\
                                      - riskf_rate * u_NN_quad_element))\
#                                    *(d1tu_NN_quad_element - self.epsilon*d2xu_NN_quad_element))\
                    ...
                    # tf.convert_to_tensor([[jacobian*tf.reduce_sum(\
                    #                 self.wquad[:,0:1]*testx_quad_element[r]*self.wquad[:,1:2]*testt_quad_element[k]
                    #                 *(d1tu_NN_quad_element + riskf_rate * x_quad_element * d1xu_NN_quad_element + 0.5*(sigma**2)*(x_quad_element**2)*d2xu_NN_quad_element\
                    #                   - riskf_rate * u_NN_quad_element))\    
                    #                 for r in range(Ntest_elementx)] for k in range(Ntest_elementt)], dtype= tf.float64)
...
###############################################################################
    ...
    def net_f(self, x, t):
        ...
        d1xu = tf.gradients(u, x)[0]
        d2xu = tf.gradients(d1xu, x)[0]
        f = d1tu + riskf_rate * x * d1xu + 0.5 * sigma * sigma * x * x * d2xu - riskf_rate * u
        return f
...
if __name__ == "__main__": 
    ...
    t_in.fill(1) #intitial condition kshitij
...
#%%    
    ...
    def u_ext(x, t, trunc = 800):
        ...
        if(x <= 0):
            return 0
        d1 = (math.log(x / K) + (riskf_rate + sigma ** 2 / 2) * T) / (sigma * math.sqrt(T))
        d2 = d1 - sigma * math.sqrt(T)
        N = lambda y: (1 + math.erf(y / math.sqrt(2))) / 2  # cumulative normal distribution
        call_value = x * N(d1) - K * math.exp(-riskf_rate * (T - t)) * N(d2)
        return call_value
        # # Series index:
...
#%% 
    ...
    cbar = fig_pred.colorbar(CS_pred, ticks = np.arange(0,100.1,20))
    cbar.ax.tick_params(labelsize = labelsize)
    ...
    fig_err, ax_err = plt.subplots(constrained_layout=True)
    CS_err = ax_err.contourf(y_test_plot, x_test_plot, abs(z_test_plot - z_pred_plot)/100, 100, cmap='jet', origin='lower')
    ...
    #fig.tight_layout()
    fig_err.set_size_inches(w=11,h=11)
    plt.savefig(''.join(['black_scholes','_PntErr','.png']))
    
    
    fig = plt.figure()
    ax = plt.axes(projection = '3d')
    ax.plot_surface(y_test_plot, x_test_plot, abs(z_test_plot - z_pred_plot)/60, rstride=1, cstride=1,cmap='viridis', edgecolor='none')
    plt.savefig('3d_error')

    fig = plt.figure()
    ax = plt.axes(projection = '3d')
    ax.plot_surface(y_test_plot, x_test_plot, z_test_plot, rstride=1, cstride=1,cmap='viridis', edgecolor='none')
    ax.set_xlabel('t')
    ax.set_ylabel('S')
    ax.set_title('Exact value to European call option')
    plt.savefig('3d_exact')

    fig = plt.figure()
    ax = plt.axes(projection = '3d')
    ax.plot_surface(y_test_plot, x_test_plot, z_test_plot + abs(z_test_plot - z_pred_plot), rstride=1, cstride=1,cmap='viridis', edgecolor='none')
    ax.set_xlabel('t')
    ax.set_ylabel('S')
    ax.set_title('Predicted value to European call option')
    plt.savefig('3d_pred')
    ...
    fig, ax = plt.subplots()
    cs = ax.contourf(y_test_plot, x_test_plot, abs(z_test_plot - z_pred_plot), cmap='viridis')
